...
export const ReportActivitiesTeacher = ({ activities }: Props) => {
  return (
    <Document>
      <Page style={styles.page} size="A4">
        <View style={styles.section}>
          ...
          <Text style={styles.header}>REPORTE DE ACTIVIDADES</Text>
          ...
          <View style={styles.table}>
            <View style={styles.tableRow}>
              ...
              <Text style={styles.tableColHeaderMinCed}>HORARIO DE CLASES</Text>
            ...
            {activities.length === 0 ? (
              <Image src={data} style={{ height: "80px" }} />
            ) : (
              Array.from(activities).map((item, index) => (
                <View style={styles.tableRow} key={index}>
                  <Text style={styles.tableColMin}>{index}</Text>
                  <Text style={styles.tableColMinCed}>
                    {item.schedule.time.startTime +
                      "-" +
                      item.schedule.time.endTime}
                  </Text>
                  <Text style={styles.tableColMinCed}>
